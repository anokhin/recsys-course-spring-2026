...
### Результаты A/B
Локально перед сабмитом я прогнал A/B-тест с закреплением пользователей за группами на 8000 эпизодов с тем же `seed=31312`, что используется в Makefile. Метрика считалась так же, как в `analyze_ab.py`: сначала агрегирование на уровне пользователей, потом сравнение контрольной и экспериментальной групп по `mean_time_per_session` через доверительный интервал Уэлча. Прирост получился большим и статистически значимым, поэтому итоговая проверка на GitHub должна воспроизвести победу уже на 30000 эпизодов.
