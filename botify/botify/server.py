...
content_rerank_recommender = ContentRerankRecommender(
    listen_history_redis=listen_history_redis.connection,
    i2i_redis=recommendations_contextual_redis.connection,
    embeddings_path=app.config["CONTENT_EMBEDDINGS_PATH"],
    meta_path=app.config["CONTENT_EMBEDDINGS_META_PATH"],
...
class NextTrack(Resource):
    def post(self, user: int):
        ...
        treatment = Experiments.CONTENT_RERANK.assign(user)

        if treatment == Treatment.C:
            recommender = sasrec_i2i_recommender
        elif treatment == Treatment.T1:
            recommender = content_rerank_recommender
        else:
