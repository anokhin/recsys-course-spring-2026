...
from botify.recommenders.map_session import MAPSession
...
map_session_recommender = MAPSession(
    listen_history_redis.connection,
    recommendations_contextual_redis.connection,
    app.config["SESSION_EMBEDDINGS_FILE_PATH"],
...
class NextTrack(Resource):
    def post(self, user: int):
        ...
        treatment = Experiments.MAP_SESSION.assign(user)

        if treatment == Treatment.C:
            recommender = sasrec_i2i_recommender
        elif treatment == Treatment.T1:
            recommender = map_session_recommender
        else:
