...
class SessionSemanticRecommender(Recommender):
    ...
    def recommend_next(self, user: int, prev_track: int, prev_track_time: float) -> int:
        ...
        history_tracks = np.array([track for track, _ in history], dtype=np.int32)
        weights = np.array(
            [max(float(listened_time), self.min_weight) for _, listened_time in history],
            dtype=np.float32,
        )

        profile = (self.item_vectors[history_tracks] * weights[:, None]).sum(axis=0) / weights.sum()
        profile /= np.linalg.norm(profile) + 1e-8

        candidate_pool = np.unique(self.neighbors[history_tracks].reshape(-1))
        seen_tracks = set(history_tracks.tolist())
        ...
        scores -= self.artist_penalty * artist_counts[self.track_artist_ids[candidate_pool]]
